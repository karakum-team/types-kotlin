// Automatically generated - do not modify!

@file:Suppress(
"NESTED_CLASS_IN_EXTERNAL_INTERFACE",
)

package web.gl

import seskar.js.JsValue
import seskar.js.JsVirtual

        @JsVirtual
        external sealed interface WebGLExtension {
            companion object {
                @JsValue("ANGLE_instanced_arrays")
val ANGLE_instanced_arrays : WebGLExtension.ANGLE_instanced_arrays
@JsValue("EXT_blend_minmax")
val EXT_blend_minmax : WebGLExtension.EXT_blend_minmax
@JsValue("EXT_color_buffer_float")
val EXT_color_buffer_float : WebGLExtension.EXT_color_buffer_float
@JsValue("EXT_color_buffer_half_float")
val EXT_color_buffer_half_float : WebGLExtension.EXT_color_buffer_half_float
@JsValue("EXT_float_blend")
val EXT_float_blend : WebGLExtension.EXT_float_blend
@JsValue("EXT_frag_depth")
val EXT_frag_depth : WebGLExtension.EXT_frag_depth
@JsValue("EXT_sRGB")
val EXT_sRGB : WebGLExtension.EXT_sRGB
@JsValue("EXT_shader_texture_lod")
val EXT_shader_texture_lod : WebGLExtension.EXT_shader_texture_lod
@JsValue("EXT_texture_compression_bptc")
val EXT_texture_compression_bptc : WebGLExtension.EXT_texture_compression_bptc
@JsValue("EXT_texture_compression_rgtc")
val EXT_texture_compression_rgtc : WebGLExtension.EXT_texture_compression_rgtc
@JsValue("EXT_texture_filter_anisotropic")
val EXT_texture_filter_anisotropic : WebGLExtension.EXT_texture_filter_anisotropic
@JsValue("KHR_parallel_shader_compile")
val KHR_parallel_shader_compile : WebGLExtension.KHR_parallel_shader_compile
@JsValue("OES_element_index_uint")
val OES_element_index_uint : WebGLExtension.OES_element_index_uint
@JsValue("OES_fbo_render_mipmap")
val OES_fbo_render_mipmap : WebGLExtension.OES_fbo_render_mipmap
@JsValue("OES_standard_derivatives")
val OES_standard_derivatives : WebGLExtension.OES_standard_derivatives
@JsValue("OES_texture_float")
val OES_texture_float : WebGLExtension.OES_texture_float
@JsValue("OES_texture_float_linear")
val OES_texture_float_linear : WebGLExtension.OES_texture_float_linear
@JsValue("OES_texture_half_float")
val OES_texture_half_float : WebGLExtension.OES_texture_half_float
@JsValue("OES_texture_half_float_linear")
val OES_texture_half_float_linear : WebGLExtension.OES_texture_half_float_linear
@JsValue("OES_vertex_array_object")
val OES_vertex_array_object : WebGLExtension.OES_vertex_array_object
@JsValue("OVR_multiview2")
val OVR_multiview2 : WebGLExtension.OVR_multiview2
@JsValue("WEBGL_color_buffer_float")
val WEBGL_color_buffer_float : WebGLExtension.WEBGL_color_buffer_float
@JsValue("WEBGL_compressed_texture_astc")
val WEBGL_compressed_texture_astc : WebGLExtension.WEBGL_compressed_texture_astc
@JsValue("WEBGL_compressed_texture_etc")
val WEBGL_compressed_texture_etc : WebGLExtension.WEBGL_compressed_texture_etc
@JsValue("WEBGL_compressed_texture_etc1")
val WEBGL_compressed_texture_etc1 : WebGLExtension.WEBGL_compressed_texture_etc1
@JsValue("WEBGL_compressed_texture_pvrtc")
val WEBGL_compressed_texture_pvrtc : WebGLExtension.WEBGL_compressed_texture_pvrtc
@JsValue("WEBGL_compressed_texture_s3tc")
val WEBGL_compressed_texture_s3tc : WebGLExtension.WEBGL_compressed_texture_s3tc
@JsValue("WEBGL_compressed_texture_s3tc_srgb")
val WEBGL_compressed_texture_s3tc_srgb : WebGLExtension.WEBGL_compressed_texture_s3tc_srgb
@JsValue("WEBGL_debug_renderer_info")
val WEBGL_debug_renderer_info : WebGLExtension.WEBGL_debug_renderer_info
@JsValue("WEBGL_debug_shaders")
val WEBGL_debug_shaders : WebGLExtension.WEBGL_debug_shaders
@JsValue("WEBGL_depth_texture")
val WEBGL_depth_texture : WebGLExtension.WEBGL_depth_texture
@JsValue("WEBGL_draw_buffers")
val WEBGL_draw_buffers : WebGLExtension.WEBGL_draw_buffers
@JsValue("WEBGL_lose_context")
val WEBGL_lose_context : WebGLExtension.WEBGL_lose_context
@JsValue("WEBGL_multi_draw")
val WEBGL_multi_draw : WebGLExtension.WEBGL_multi_draw
            }
            
            sealed interface ANGLE_instanced_arrays : WebGLExtension
sealed interface EXT_blend_minmax : WebGLExtension
sealed interface EXT_color_buffer_float : WebGLExtension
sealed interface EXT_color_buffer_half_float : WebGLExtension
sealed interface EXT_float_blend : WebGLExtension
sealed interface EXT_frag_depth : WebGLExtension
sealed interface EXT_sRGB : WebGLExtension
sealed interface EXT_shader_texture_lod : WebGLExtension
sealed interface EXT_texture_compression_bptc : WebGLExtension
sealed interface EXT_texture_compression_rgtc : WebGLExtension
sealed interface EXT_texture_filter_anisotropic : WebGLExtension
sealed interface KHR_parallel_shader_compile : WebGLExtension
sealed interface OES_element_index_uint : WebGLExtension
sealed interface OES_fbo_render_mipmap : WebGLExtension
sealed interface OES_standard_derivatives : WebGLExtension
sealed interface OES_texture_float : WebGLExtension
sealed interface OES_texture_float_linear : WebGLExtension
sealed interface OES_texture_half_float : WebGLExtension
sealed interface OES_texture_half_float_linear : WebGLExtension
sealed interface OES_vertex_array_object : WebGLExtension
sealed interface OVR_multiview2 : WebGLExtension
sealed interface WEBGL_color_buffer_float : WebGLExtension
sealed interface WEBGL_compressed_texture_astc : WebGLExtension
sealed interface WEBGL_compressed_texture_etc : WebGLExtension
sealed interface WEBGL_compressed_texture_etc1 : WebGLExtension
sealed interface WEBGL_compressed_texture_pvrtc : WebGLExtension
sealed interface WEBGL_compressed_texture_s3tc : WebGLExtension
sealed interface WEBGL_compressed_texture_s3tc_srgb : WebGLExtension
sealed interface WEBGL_debug_renderer_info : WebGLExtension
sealed interface WEBGL_debug_shaders : WebGLExtension
sealed interface WEBGL_depth_texture : WebGLExtension
sealed interface WEBGL_draw_buffers : WebGLExtension
sealed interface WEBGL_lose_context : WebGLExtension
sealed interface WEBGL_multi_draw : WebGLExtension
        }
